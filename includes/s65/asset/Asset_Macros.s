...
.macro Asset_Preload() {
	.if(Asset_PreloaderList.size() > 0) {
		...
		.for(var i=0; i<Asset_PreloaderList.size(); i++) {
			...
			.if(asset.get("addr") < $60000) {
				SDCard_LoadToChipRam  asset.get("addr") : asset.get("filename")
			} else {
				SDCard_LoadToAtticRam  asset.get("addr") : asset.get("filename")
			}
...
*
* Loads an external asset previosuly defined using Asset_AddExternal or Asset_StartExternal
* from SDcard into the memory address provided
* 
* @namespace Asset
*
* @param {string} name The Asset name defined in the Asset_AddExternal or Asset_StartExternal macro call
* @param {dword} address The address to load this asset into
* @flags nzc
*/
.macro Asset_LoadFromExternal(name,address){
	S65_AddToMemoryReport("Asset_LoadFromExternal")
	S65_SaveRegisters()
		.for(var i=0; i< Asset_ExternalAssetList.size(); i++) {
			.if(Asset_ExternalAssetList.get(i).get("name") == name) {
				.if(address < $5ffff) {   
					SDCard_LoadToChipRam address : Asset_ExternalAssetList.get(i).get("filename")
				} else {
					SDCard_LoadToAtticRam address : Asset_ExternalAssetList.get(i).get("filename")
				}
			}
		}
...
// .if(preload) {
// 		.eval PC = *
// 		.if(SpriteMetaPreloadIndex == -1) {
// 			.var name = "p"+Asset_PreloaderList.size()
// 			.eval SpriteMetaPreloadIndex = Asset_PreloaderList.size()
// 			Asset_PreloaderFetchSegment(SpriteMetaPreloadIndex)
// 			.eval Asset_PreloaderList.add(Hashtable().put(
// 				"name", name,
// 				"addr", $80f0000,
// 				"filename", Asset_PreloaderFilenamePointer 
...
// 		Asset_PreloaderSegment(SpriteMetaPreloadIndex)
...
* .macro StartExternal
*
* Signals the start of a blcok of data/code that will be saved to the SDCard
* for importing at runtime.<br>
* NOTE: If you are using this to save code make sure to use .pseudopc directive
* to set your intended load address as the program counter will be reset to $0000 for this
* 
* @namespace Asset
*
* @param {string} name The name to reference this file
*/
.macro Asset_StartExternal(name) {
		.var fname = "s"+Asset_ExternalAssetList.size()
		.var fnamePointer = Asset_PreloaderFilenamePointer
		...
		.eval Asset_ExternalAssetList.add(Hashtable().put(
			"name", name,
			"filename", fnamePointer 
		))
		S65_Trace("Adding "+name+" external data/code block to the SDCard")
}

/**
* .macro EndExternal
*
* Ends the data/code block that was previously started with an Asset_StartExternal and
* returns the program counter to where it was prior to starting the block.
* 
* @namespace Asset
*/
.macro Asset_EndExternal() {
		.segment S65Code
}