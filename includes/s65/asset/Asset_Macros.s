...
*/
.macro Asset_ImportTilemap(name, path, address) {
		//Add an entry to the buildtime spriteset list
		//{ id, name, address, colorAddress, palette, colors }
		...
		.if(address >= S65_HIGHEST_LOAD) {
			...
			Asset_PreloaderFetchSegment(Asset_PreloaderList.size())
				...
				.print("Adding "+name+" tilemap asssets to the preloader")
			.segment S65Code
		...
		.if(!preload) {
			* = address "Tilemap importing"
			...
			.eval tilemap.tiledefAddress = address + bin.getSize()
		}
        ...
        .if(!preload) {
        	...
        	* = PC "Tilemap import"
        	...
        	.eval S65_LastImportPtr = tilemap.tiledefAddress + bin.getSize()
        }
